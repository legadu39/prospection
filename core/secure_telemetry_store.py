...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s | %(levelname)-8s | [TELEMETRY] %(message)s"
)
...
class NexusDB:
    ...
    PRIVACY_SALT = settings.PRIVACY_SALT if hasattr(settings, 'PRIVACY_SALT') else "NEXUS_GDPR_COMPLIANCE_V1_SALT"
    ...
    def session(self, immediate: bool = False):
        ...
        for attempt in range(max_retries):
            try:
                ...
                is_sqlite_lock = isinstance(e, sqlite3.OperationalError) and "locked" in str(e).lower()
                            ...
                            if isinstance(data, list):
                                for s in data:
                                    conn.execute('''
                                        ...
                                        VALUES (?, ?, ?, ?, ?, ?, ?, 1)
                                    ''', (s.get('id'), s.get('label'), s.get('program'), s.get('ref_link'), s.get('ref_code'), s.get('priority', 2), s.get('monthly_limit_hard', 3)))
                    except Exception: pass
                            ...
                            if isinstance(data, list):
                                for c in data:
                                    conn.execute('''
                                        ...
                                        VALUES (?, ?, ?, ?, ?, ?)
                                    ''', (c.get('program'), c.get('amount'), c.get('is_boosted', 0), c.get('end_date'), c.get('url')))
                    except Exception: pass
        except Exception: pass
        ...
        try:
            with self.session() as conn:
                ...
                try:
                    ...
                    row = c.fetchone()
                    if self.use_postgres: current_ver = row['v'] if row and row['v'] else 0
                    else: current_ver = row[0] if row and row[0] else 0
                except: current_ver = 0

                if current_ver < self.CURRENT_SCHEMA_VERSION:
                    logger.info(f"🛠️ Schema Migration ({current_ver} -> {self.CURRENT_SCHEMA_VERSION})...")
    ...
    def _reset_stuck_dispatches(self, conn):
        try:
            conn.execute("UPDATE leads SET status='QUALIFIED', assigned_sponsor_id=NULL WHERE status='DISPATCHING'")
        except: pass
    ...
    def _apply_migrations(self, conn, current_ver: int):
        ...
        if current_ver < 1:
            ...
            ''')
            conn.execute("CREATE INDEX IF NOT EXISTS idx_leads_flow ON leads(status, source, created_at);")
            conn.execute('''
            ...
            ''')
            conn.execute("CREATE INDEX IF NOT EXISTS idx_logs_calc ON dispatch_logs(sponsor_id, dispatched_at_iso);")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (1, ?)", (ts_now,))

        if current_ver < 2:
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sponsors_perf ON sponsors(program, active, priority);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_leads_ai ON leads(ai_status);")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (2, ?)", (ts_now,))
        ...
        if current_ver < 3:
            ...
            ''')
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (3, ?)", (ts_now,))
        ...
        if current_ver < 6:
            ...
            ''')
            conn.execute("CREATE INDEX IF NOT EXISTS idx_viral_queue ON viral_queue(status, priority DESC, created_at ASC);")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (6, ?)", (ts_now,))
        ...
        if current_ver < 7:
            ...
            ''')
            conn.execute("CREATE INDEX IF NOT EXISTS idx_accounts_status ON accounts(platform, status);")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (7, ?)", (ts_now,))
        ...
        if current_ver < 8:
            ...
            ''')
            conn.execute("CREATE INDEX IF NOT EXISTS idx_conversions_sponsor ON conversions(sponsor_id);")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (8, ?)", (ts_now,))
        ...
        if current_ver < 9:
            ...
            ''')
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (9, ?)", (ts_now,))
            
        if current_ver < 10:
            try:
                col_type = "JSONB" if self.use_postgres else "TEXT"
                conn.execute(f"ALTER TABLE leads ADD COLUMN meta_analysis {col_type} DEFAULT '{{}}'")
            except Exception as e:
                logger.warning(f"⚠️ Migration V10 warning (column exists?): {e}")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (10, ?)", (ts_now,))
        ...
        if current_ver < 11:
            try:
                ...
                ''')
                conn.execute("CREATE INDEX IF NOT EXISTS idx_reputation_status ON author_reputation(status, expiry_ts);")
            except Exception as e:
                logger.warning(f"⚠️ Migration V11 warning: {e}")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (11, ?)", (ts_now,))
        ...
        if current_ver < 12:
            ...
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (12, ?)", (ts_now,))
        ...
        if current_ver < 13:
            ...
            try:
                ...
                ''')
                conn.execute("CREATE INDEX IF NOT EXISTS idx_ledger_partner ON ledger(partner_id, transaction_date);")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_ledger_ref ON ledger(reference_external);")
            except Exception as e:
                logger.warning(f"⚠️ Migration V13 warning: {e}")
            conn.execute("INSERT INTO _schema_version (version, updated_at) VALUES (13, ?)", (ts_now,))
    ...
    def to_decimal(self, value: Any) -> Decimal:
        try:
            if value is None: return Decimal("0.0")
    ...
    def _hash_identity(self, raw_id: str) -> str:
        ...
        """
        if not raw_id or raw_id == "unknown": return "anonymous_node"
    ...
    def get_system_health(self) -> float:
        try:
            ...
            if backlog > 500: health_score += 0.6
            elif backlog > 200: health_score += 0.3
            
            if self.last_write_latency > 2.0: health_score += 0.4
            elif self.last_write_latency > 1.0: health_score += 0.2
    ...
    def register_conversion_event(self, lead_id: str, amount: float = 0.0, transaction_id: str = None) -> bool:
        """
        ...
        try:
            ...
            with self.session(immediate=True) as conn:
                res = conn.execute("SELECT assigned_sponsor_id, status, program, assigned_program, author, ai_process_info, source, url FROM leads WHERE id=?", (lead_id,))
                row = res.fetchone()
                
                if not row: return False
                
                sponsor_id = row['assigned_sponsor_id'] if self.use_postgres else row['assigned_sponsor_id']
                ...
                conn.execute("UPDATE leads SET status='CONVERTED', updated_at=? WHERE id=?", (now, lead_id))
                ...
                NEXT_STEP_MATRIX = {
                    ...
                    "BOURSOBANK": {"target": "MERIA", "delay_hours": 48, "tag": "CROSS_SELL_CRYPTO"},
                    "FORTUNEO": {"target": "BINANCE", "delay_hours": 24, "tag": "CROSS_SELL_CRYPTO"},
                    "HELLOBANK": {"target": "TRADE_REPUBLIC", "delay_hours": 24, "tag": "CROSS_SELL_INVEST"},
                ...
                if next_step:
                    ...
                    logger.info(f"🔄 INTELLIGENT WORKFLOW: Triggering '{target_program}' for user after '{current_program}' success.")
                    
                    # Prepare Context for the new Lead
                    ai_info = row['ai_process_info']
                    try:
                        info_dict = json.loads(ai_info) if isinstance(ai_info, str) else (ai_info or {})
                    except: info_dict = {}
    ...
    def get_active_fleet(self, platform: str = 'tiktok') -> List[Dict]:
        try:
            with self.session() as conn:
                ...
                if self.use_postgres: return [dict(r) for r in rows]
                else: return [dict(r) for r in rows]
        except Exception as e:
            logger.error(f"❌ Fleet Retrieval Error: {e}")
            return []

    def register_account_heartbeat(self, account_id: str, proxy_url: str = None, status: str = 'ACTIVE'):
        try:
            now = time.time()
            with self.session() as conn:
                if proxy_url:
        ...
        try:
            with self.session(immediate=True) as conn:
                ...
                if row:
                    ...
                    conn.execute("UPDATE viral_queue SET status='PROCESSED', updated_at=? WHERE username=?", (time.time(), username))
                    return username
                return None
        except Exception as e:
    ...
    def analyze_user_history(self, author_id: str, hours: int = 24) -> Dict[str, Any]:
        ...
        """
        if not author_id: return {"status": "NEW", "action": "PROCESS"}
        ...
        try:
            ...
            with self.session() as conn:
                ...
                """, (lookup_id, vip_cutoff)).fetchall()
                
                leads = [dict(r) for r in rows]
                
                conversions = [l for l in leads if l['status'] == 'CONVERTED']
                if conversions: return {"status": "VIP_USER", "action": "PRIORITY_DISPATCH", "reason": "PAST_SUCCESS"}

                count_recent = len([l for l in leads if l['created_at'] > cutoff])
                failures = [l for l in leads if 'FAILED' in l['status']]
                
                if len(failures) > 2: return {"status": "DEAD_END", "action": "HARD_BLOCK", "reason": "TOO_MANY_FAILURES"}
                
                if count_recent == 1:
                    prev = leads[0]
                    if 'FAILED' not in prev['status']:
                        return {"status": "HOT_RETURN", "action": "BOOST_SCORE", "prev_context": prev.get('program')}
                
                if count_recent >= 2: return {"status": "SPAM", "action": "BLOCK"}
    ...
    def get_program_leaderboard(self, program: str) -> List[Dict]:
        try:
            with self.session() as conn:
                ...
                ranked_list = []
                for s in sponsors:
                    balance = float(s['balance_available'] if s['balance_available'] is not None else 0.0)
    ...
    def process_wallet_topup(self, sponsor_id: str, amount_euros: float, payment_ref: str, source: str = "STRIPE") -> bool:
        """
        ...
        try:
            ...
            with self.session(immediate=True) as conn:
                check = conn.execute("SELECT id FROM ledger WHERE reference_external = ?", (payment_ref,)).fetchone()
                if check:
                ...
                row = conn.execute("SELECT balance_available FROM sponsors WHERE id=?", (sponsor_id,)).fetchone()
    ...
    def atomic_dispatch_transaction(self, lead_id: str, sponsor_id: str, program: str, estimated_cost: Decimal) -> bool:
        """
        ...
        try:
            ...
            with self.session(immediate=True) as conn:
                ...
                if not row_sponsor: return False
                ...
                if balance_avail >= cost_decimal:
                    ...
                    logger.warning(f"📉 SOLVENCY CHECK FAILED: Sponsor {sponsor_id} lacks funds ({balance_avail} < {cost_decimal}).")
                
                # --- CAPACITY GUARD ---
                # Legacy Quota Support (Deprecated in V29 - Kept for fallback)
                if verified < limit and limit > 0:
                     logger.info(f"📊 LEGACY QUOTA USED for {sponsor_id} (No funds, but within limits).")
                     conn.execute('''
        ...
        try:
            with self.session(immediate=True) as conn:
                ...
                conn.execute("""
                    ...
                    VALUES (?, 'CONSUMPTION', ?, ?, ?, ?, ?)
                """, (time.time(), sponsor_id, -cost, lead_id, "Resource Consumption (SLA Met)", time.time()))
    ...
    def insert_raw_lead(self, lead: Dict[str, Any], conn=None) -> bool:
        ...
        try:
            ...
            if isinstance(existing_info, str):
                try: existing_info = json.loads(existing_info)
                except: existing_info = {}
    ...
    def reserve_leads_for_dispatch(self, batch_size: int = 50, batch_id: str = "batch_temp") -> List[Dict]:
        """
        ...
        try:
            ...
            with self.session(immediate=True) as conn:
                ...
                for row in rows:
                    ...
                    conn.execute("UPDATE leads SET status='DISPATCHING', ai_process_info=?, updated_at=? WHERE id=?", 
    ...
    def close(self):
        ...
        if self.use_postgres and NexusDB._pg_pool:
            try: NexusDB._pg_pool.closeall()
            except: pass

    def mark_lead_sent(self, lead_id: str):
        try:
            with self.session() as conn:
                conn.execute("UPDATE leads SET status='SENT', updated_at=? WHERE id=?", (time.time(), lead_id))
        except: pass

    def fail_lead(self, lead_id: str, reason: str = "ERROR"):
        try:
            with self.session() as conn:
                conn.execute("UPDATE leads SET status=?, updated_at=? WHERE id=?", (f'FAILED_{reason}', time.time(), lead_id))
    ...
    def get_program_sponsors_stats(self, program: str) -> List[Dict]:
        try:
            with self.session() as conn:
                iso_start = datetime.now().replace(day=1, hour=0, minute=0, second=0, microsecond=0).isoformat()
                res = conn.execute('''
    ...
    def get_backlog_count(self) -> int:
        try:
            with self.session() as conn:
                r = conn.execute("SELECT COUNT(*) as cnt FROM leads WHERE status='QUALIFIED' OR status='NEW'").fetchone()
    ...
    def get_max_active_campaign_amount(self) -> float:
        try:
            with self.session() as conn:
                r = conn.execute("SELECT MAX(amount) as m FROM campaigns WHERE end_date > ?", (datetime.now().isoformat(),)).fetchone()
    ...
    def get_next_lead_to_send_by_source(self, source: str) -> Optional[Dict]:
        try:
            with self.session(immediate=True) as conn:
                ...
                if row:
                    lead = dict(row)
                    conn.execute("UPDATE leads SET status='SENDING', updated_at=? WHERE id=?", (time.time(), lead['id']))
                    return lead
                return None
        except: return None
    ...
    def get_converted_leads_sample(self, limit: int = 500) -> List[Dict]:
        try:
            with self.session() as conn:
                rows = conn.execute("SELECT comment_text FROM leads WHERE status='CONVERTED' LIMIT ?", (limit,)).fetchall()
    ...
    def get_subreddit_stats(self, subreddit: str) -> Dict[str, Any]:
        try:
            with self.session() as conn:
                row = conn.execute("SELECT * FROM subreddit_stats WHERE subreddit=?", (subreddit,)).fetchone()
                return dict(row) if row else {"success_count": 0, "fail_count": 0, "last_fail_ts": 0}
        except: 
            return {"success_count": 0, "fail_count": 0, "last_fail_ts": 0}
            
    def upsert_campaign(self, program: str, amount: float, is_boosted: bool, end_date: str, url: str):
        try:
            now = time.time()
            with self.session() as conn:
                conn.execute('''
    ...
    def update_author_reputation(self, author_id: str, platform: str, score_delta: int, new_status: str = None):
        """
        ...
        try:
            ...
            with self.session() as conn:
                ...
                if existing:
                    current_score = existing['reputation_score'] if self.use_postgres else existing[0]
    ...
    def get_sponsor_stats(self, sponsor_id: str) -> Dict[str, Any]:
        ...
        try:
            with self.session() as conn:
                iso_start = datetime.now().replace(
                ...
                if self.use_postgres:
                    ...
                    return {
                        "verified_count_month": row[0] or 0,
                        "pending_leads_count": row[1] or 0
                    }
        except Exception as e:
    ...
    def get_dashboard_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with self.session() as conn:
                ...
                res_cost = conn.execute("SELECT SUM(amount) as total FROM ledger WHERE type='CONSUMPTION'").fetchone()
                ...
                # 3. Leads Counts
                res_leads = conn.execute("SELECT program, COUNT(*) as cnt FROM leads WHERE status='CONVERTED' GROUP BY program").fetchall()
                for row in res_leads:
                    prog = (row['program'] if self.use_postgres else row[0]).upper()
                    cnt = (row['cnt'] if self.use_postgres else row[1])
                    if "BOURSO" in prog or "BANK" in prog: snapshot["leadsBank"] += cnt
                    elif "MERIA" in prog or "CRYPTO" in prog: snapshot["leadsCrypto"] += cnt
                    elif "SAAS" in prog: snapshot["leadsSaaS"] += cnt

                # 4. Stocks (Partner Capacities)
                res_stocks = conn.execute("SELECT SUM(monthly_limit_hard - verified_count_month) as remaining FROM sponsors WHERE active=1").fetchone()
                snapshot["stocks"]["partnerCap"] = (res_stocks['remaining'] if self.use_postgres else res_stocks[0]) or 0
                
                # 5. Bots Health (Derived from Account Statuses)
                res_bots = conn.execute("SELECT platform, status, last_active_ts FROM accounts").fetchall()
                platforms = {}
                for row in res_bots:
                    ...
                    if plat not in platforms: platforms[plat] = {"total":0, "active":0, "last_ts": 0}
                    platforms[plat]["total"] += 1
                    if stat == 'ACTIVE': platforms[plat]["active"] += 1
                    if last_ts > platforms[plat]["last_ts"]: platforms[plat]["last_ts"] = last_ts

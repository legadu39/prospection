...
async def startup_event():
    ...
    try:
        with db.session() as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_attr_hash ON attribution_history(user_hash)"
            )
...
def get_persistent_history(user_hash: str) -> List[str]:
    ...
    try:
        with db.session() as conn:
            rows = conn.execute(
                "SELECT mandate_id FROM attribution_history WHERE user_hash = ?", (user_hash,)
            ).fetchall()
...
def record_conversion_event(
    lead_id: str, mandate_id: str, ip: str, user_agent: str, was_failover: bool = False
):
    """
    ...
    try:
        ...
        if mandate_id not in PARTNER_PERFORMANCE:
            PARTNER_PERFORMANCE[mandate_id] = {
                "impressions": 0,
                "conversions": 0,
...
def allocate_inventory_to_mandate(
    requested_mandate_id: str, exclude_ids: List[str] = [], country_code: str = "GLOBAL"
    ...
    try:
        with db.session() as conn:
            ...
            if target:
                ...
                try:
                    allowed_geos = (
                        json.loads(allowed_geos_json) if allowed_geos_json else ["GLOBAL"]
                    )
                except Exception:
                    allowed_geos = ["GLOBAL"]
                ...
                if "GLOBAL_EXCEPT_US_FR" in allowed_geos and country_code in ["US", "FR"]:
                    logger.info(
                        f"🌍 GEO BLOCKED: {country_code} excluded from {requested_mandate_id}."
                    )
                    ...
                    if country_code not in allowed_geos:
                        logger.info(
                            f"🌍 GEO BLOCKED: {country_code} not in allowed list for {requested_mandate_id}."
                        )
                        needs_reallocation = True
                ...
                if country_code == "FR" and "PROP_FIRM" in prog_type:
                    logger.warning(
                        f"🛡️ AMF COMPLIANCE SHIELD: Blocking Prop Firm {requested_mandate_id} for FR user. Rerouting to SaaS."
                    )
                    needs_reallocation = True
            ...
            if requested_mandate_id in exclude_ids:
                ...
                balance = db.to_decimal(
                    target["balance_available"] if db.use_postgres else target["balance_available"]
                )
                ...
                if balance < 15.0:
                    logger.warning(
                        f"📉 BUDGET CONTROL: Mandate {target['id']} paused (Funds: {balance}€). Reallocating."
                    )
                    needs_reallocation = True
                elif limit > 0 and verified >= limit:
                    logger.info(
                        f"📊 QUOTA REACHED: Mandate {target['id']} fulfilled ({verified}/{limit})."
                    )
                    needs_reallocation = True
            ...
            if not needs_reallocation:
                return (
                    target["url_template"] or fallback_url,
                    target["id"],
                    False,
                    target["program"],
                )
            ...
            logger.info(
                f"🔄 INVENTORY REALLOCATION triggered for {requested_mandate_id} (Geo: {country_code})"
            )
            ...
            for c in candidates:
                ...
                if "GLOBAL_EXCEPT_US_FR" in allowed and country_code in ["US", "FR"]:
                    continue
                ...
                if country_code == "FR" and "PROP_FIRM" in str(c["program"]).upper():
                    continue

                bal = db.to_decimal(
                    c["balance_available"] if db.use_postgres else c["balance_available"]
                )
                if bal < 15.0:
                    continue
            ...
            for cand in valid_candidates:
                ...
                bal = float(
                    cand["balance_available"] if db.use_postgres else cand["balance_available"]
                )
                ...
                if is_euro_traffic:
                    if program_type == "CRYPTO_PSAN":
                        geo_weight = 5.0  # Priorité absolue FR
                    elif program_type == "SAAS":
                        geo_weight = 3.0
                elif is_anglo_traffic:
                    if program_type == "PROP_FIRM":
                        geo_weight = 4.0  # Priorité absolue US
                else:
                    if program_type == "CRYPTO_EXCHANGE":
                        geo_weight = 3.0

                # Intelligence V3.3: Pacing Factor Prédictif
                pacing_multiplier = calculate_pacing_factor(
                    cand["verified_count_month"], cand["monthly_limit_hard"], cand["program"]
                )
            ...
            if best_candidate:
                return (
                    best_candidate["url_template"] or fallback_url,
                    best_candidate["id"],
                    True,
...
async def health_check():
    """
    Healthcheck pour Docker / K8s / load-balancer.
    Vérifie la connectivité DB et le nombre de sponsors actifs.
    Retourne HTTP 200 si tout est opérationnel, 503 sinon.
    """
    try:
        sponsors_loaded = 0
        with db.session() as conn:
            row = conn.execute("SELECT COUNT(*) as cnt FROM sponsors WHERE active = 1").fetchone()
            sponsors_loaded = row["cnt"] if db.use_postgres else row[0]
        ...
        return JSONResponse(
            ...
            content={
                "status": "ok",
                "db": "connected",
                "sponsors_loaded": sponsors_loaded,
            },
        )
    except Exception as e:
...
async def show_landing_page(id: str = "demo", p: str = "default", mode: Optional[str] = None):
    if (STATIC_DIR / "index.html").exists():
        ...
        return HTMLResponse(
            "<h1>System Maintenance</h1><p>Platform updating. Please retry shortly.</p>",
            status_code=503,
        )
    ...
    try:
        with db.session() as conn:
            ...
            if rows:
                ...
                for row in rows:
                    terms = (
                        json.loads(row["financial_terms"])
                        if isinstance(row["financial_terms"], str)
                        else row["financial_terms"]
                    )
                    offers_config.append(
                        {
                            ...
                            "amount": terms.get("target_cpa", 0)
                            if terms.get("model") == "REVENUE_SHARE"
                            else 100,
                        }
                    )
...
async def preflight_allocation(lead_id: str, partner_id: str, request: Request):
    ...
    if clean_pid == "nexus_tech_access":
        ...
        return JSONResponse(
            content={"status": "ready", "token": token, "target_preview": "NEXUS_PARTNER"}
        )
    ...
    try:
        ...
        real_url, final_pid, is_realloc, prog = allocate_inventory_to_mandate(
            clean_pid, exclude_ids=user_seen, country_code=country_code
        )
        ...
        if not verify_mandate_availability(real_url):
            ...
            real_url, final_pid, is_realloc, prog = allocate_inventory_to_mandate(
                clean_pid, exclude_ids=current_excludes, country_code=country_code
            )
        ...
        try:
            with db.session() as conn:
                row = conn.execute(
                    "SELECT monthly_limit_hard, verified_count_month FROM sponsors WHERE id=?",
                    (final_pid,),
                ).fetchone()
...
async def track_and_redirect(
    ...
    if t and t in PREFETCH_CACHE:
        ...
        if exclude_pid and cached["partner_id"] == exclude_pid:
            ...
            if db.atomic_dispatch_transaction(
                safe_lead_id, cached["partner_id"], cached["program"], 0
            ):
    ...
    while attempts < max_attempts:
        url, pid, is_realloc, prog = allocate_inventory_to_mandate(
            safe_partner_id, exclude_ids=unique_excludes, country_code=country_code
        )
        ...
        if db.atomic_dispatch_transaction(safe_lead_id, pid, prog, 0):
            ...
            if req_hash in REQUEST_CACHE:
                if now - REQUEST_CACHE[req_hash][0] < CACHE_TTL:
                    is_dup = True
...
async def process_conversion_feedback(payload: AttributionModel, background_tasks: BackgroundTasks):
    ...
    def _finalize_transaction():
        success = db.register_conversion_event(
            lead_id=payload.click_id, amount=payload.amount, transaction_id=payload.transaction_id
        )
        if success:

...
class Settings(BaseSettings):
    ...
    GEOIP_PROVIDER: str = Field(
        default="ipstack", env="GEOIP_PROVIDER"
    )  # Options: ipstack, maxmind
    DEFAULT_ROUTING_ZONE: str = "GLOBAL"  # Zone de repli si échec détection

    # --- EXTERNAL SERVICES ---
    BREVO_TIMEOUT: int = 10  # seconds, HTTP timeout for Brevo API requests
    ...
    PARTNER_DASHBOARD_URL: str = Field(
        default="https://partenaires.nexus-insights.com/dashboard", env="PARTNER_DASHBOARD_URL"
    )
    ...
    def auto_scale_workers_based_on_resources(cls, v, values):
        ...
        try:
            ...
            if requested > safe_limit:
                if safe_limit < 1:
                    safe_limit = 1  # Toujours au moins 1 bot
                # Log via print car le logger n'est pas encore instancié à ce stade
                print("⚠️ INTELLIGENCE SYSTEM: Ressources limitées détectées.", flush=True)
                ...
                print(
                    f"👉 Auto-correction : {requested} -> {safe_limit} workers (Facteur limitant : {'CPU' if cpu_limit < ram_limit else 'RAM'}).",
                    flush=True,
                )
                return safe_limit
    ...
    def self_heal_chrome_path(cls, v):
        ...
        if v:
            raise ValueError(
                f"CRITICAL: Chrome binary not found at '{v}' and auto-discovery failed."
            )
    ...
    def force_postgres_for_high_concurrency(cls, v, values):
        ...
        if workers > 10 and not v:
            raise ValueError(
                "🚨 SÉCURITÉ : Impossible de lancer plus de 10 workers sur SQLite. Activez PostgreSQL ou réduisez MAX_CONCURRENT_WORKERS."
            )
        return v

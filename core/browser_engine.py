...
try:
    ...
    from core.database import NexusDB  # noqa: F401
except ImportError:
    print("ðŸ”´ CRITICAL ERROR: Modules missing in browser_engine.")
    sys.exit(1)
...
class CircuitBreaker:
    ...
    def decide_strategy(self, error_type: str) -> str:
        ...
        if error_type == "TIMEOUT" or error_type == "CONNECTION_REFUSED":
            # ProblÃ¨me rÃ©seau passif -> Retry simple, pas de pÃ©nalitÃ© immÃ©diate
            return "RETRY_SAME_NODE"

        elif error_type == "ACCESS_DENIED_403" or error_type == "BANNED":
            # ProblÃ¨me d'identitÃ© -> Changer User-Agent + IP immÃ©diatement
            return "ROTATE_IDENTITY"

        elif error_type == "DOM_ELEMENT_NOT_FOUND":
            # ProblÃ¨me structurel -> Le site a peut-Ãªtre changÃ© ou chargÃ© lentement
    ...
    def allow_request(self, resource_id: str, target_domain: str = None) -> bool:
        ...
        if target_domain and target_domain in self.site_open_circuits:
            if time.time() < self.site_open_circuits[target_domain]:
                logger.debug(
                    f"ðŸ”Œ Circuit Breaker: Domain {target_domain} is temporarily ignored."
                )
                return False
            else:
        ...
        if resource_id in self.open_circuits:
            if time.time() < self.open_circuits[resource_id]:
                logger.debug(f"ðŸ”Œ Circuit Breaker: {resource_id} is blocked.")
                return False
            else:
                # Time elapsed, attempt rehabilitation (Half-Open)
                logger.info(f"ðŸ”Œ Circuit Breaker: Rehabilitating {resource_id}")
    ...
    def record_failure(
        ...
        if strategy == "RETRY_SAME_NODE":
            logger.info(
                f"ðŸ›¡ï¸ Resilience: {error_type} detected on {resource_id}. Retrying without penalty."
            )
            return  # On n'incrÃ©mente pas le compteur d'Ã©chec pour un simple timeout occasionnel
        ...
        if is_site_error and target_domain:
            ...
            if self.site_failures[target_domain] >= self.SITE_THRESHOLD:
                ...
                logger.warning(
                    f"ðŸ”Œ CIRCUIT OPEN (SITE): {target_domain} isolated due to instability."
                )
        else:
            ...
            if strategy == "ROTATE_IDENTITY":
                ...
                logger.warning(
                    f"ðŸ›¡ï¸ Resilience: Critical Error ({error_type}) -> Immediate Isolation of {resource_id}"
                )
            ...
            if self.failures[resource_id] >= self.THRESHOLD:
                ...
                logger.warning(
                    f"ðŸ”Œ CIRCUIT OPEN (PROXY): {resource_id} isolated for {self.COOLDOWN}s after failures."
                )
...
class ProcessGuardian:
    ...
    def kill_zombies_on_port(port: int):
        logger.info(f"ðŸ›¡ï¸ Guardian: Scanning port {port}...")
        try:
            me = psutil.Process()
            ...
            if await self._acquire_lock():
                try:
                    ...
                    async with aiofiles.open(tmp, "wb") as f:
                        await f.write(salt + encrypted)
                    os.replace(tmp, self.storage_path)
                finally:
                    await self._release_lock()
        except Exception as e:
            logger.error(f"Save Vault Error: {e}")
        ...
        for c in cookies:
            ...
            if expires != -1:
                ...
                if c.get("name") in critical_names and ttl < SAFETY_MARGIN:
                    logger.warning(
                        f"ðŸª Session Vault: Critical cookie '{c.get('name')}' decaying (TTL: {int(ttl)}s). Force refresh."
                    )
                    critical_decay = True
...
class EnvironmentEmulator:
    ...
    def __init__(self, profile_dir: Path):
        ...
        self.CPU_RAM_OPTS = [(4, 8), (8, 8), (8, 16), (12, 16), (16, 32), (24, 64)]
    ...
    def _emulate_hardware_configuration(self) -> Dict:
        ...
        if ram >= 32:
            ...
            width, height = secrets.choice([(1920, 1080), (1366, 768)])
        else:
                    ...
                    try:
                        ...
                        if saved_ver_match and real_ver_match:
                            ...
                            if real_major > saved_major:
                                logger.info(
                                    f"ðŸ§¬ Profile Evolution: v{saved_major} -> v{real_major}"
                                )
...
class CompatibilityLayer:
    @staticmethod
    async def inject_shims(
        page, profile: EmulationProfile, timezone: str = "UTC", locale: str = "en-US"
    ):
        if not page:
            return
        ...
        """
        # Playwright Python ne supporte pas le paramÃ¨tre `arg` de add_init_script.
        # On injecte les valeurs directement dans le JS via un bloc const.
        ...
        # Remplace le wrapper IIFE `((data) => {...})(...)` par un bloc avec const data
        final_js = (
            f"(()=>{{ const data = {data_block};\n"
            + js.replace("((data) => {", "").rsplit("})", 1)[0]
            + "\n})()"
        )
        await page.add_init_script(script=final_js)
...
class SandboxCDPProfile:
    ...
    def __init__(
        self,
        user_id: str = "apex_user",
        profile_dir: Optional[str] = None,
        cdp_port: int = 9222,
        cdp_secondary_port: int = 9223,
    ):
        ...
        self.cdp_endpoints = [
            ...
            f"http://127.0.0.1:{cdp_secondary_port}",
        ]
    ...
    async def _validate_network_path(self, proxy_url: str) -> bool:
        ...
        try:
            logger.info(f"ðŸ›¡ï¸ PATH VALIDATION: Analyzing ingress for {self.user_id}...")
            async with aiohttp.ClientSession() as session:
                start = time.time()
                try:
                    async with session.get(
                        "http://ip-api.com/json/", proxy=proxy_url, timeout=15
                    ) as resp:
                        if resp.status != 200:
                            logger.error(f"âŒ PROXY DEAD ({resp.status}).")
                            # Intelligence: Distinguish HTTP Errors
                            error_type = (
                                "ACCESS_DENIED_403" if resp.status == 403 else "GENERIC_HTTP_ERROR"
                            )
                        ...
                        if any(b in isp for b in bad_isps):
                            logger.warning(
                                f"âš ï¸ DATACENTER DETECTED: IP {isp} rejected (Quality Assurance)."
                            )
                        ...
                        if cc == "FR":
                            self.proxy_metadata["locale"] = "fr-FR"
                        elif cc == "DE":
                            self.proxy_metadata["locale"] = "de-DE"
                        elif cc == "ES":
                            self.proxy_metadata["locale"] = "es-ES"
                        elif cc == "GB":
                            self.proxy_metadata["locale"] = "en-GB"

                        logger.info(
                            f"âœ… PATH CERTIFIED: {data.get('query')} ({cc}) - Latency: {elapsed:.2f}s - Zone: {self.proxy_metadata['timezone']}"
                        )
                        self.circuit_breaker.record_success(proxy_url)
                        return True
                except asyncio.TimeoutError:
                    logger.error("âŒ PROXY TIMEOUT: Latency too high.")
                    self.circuit_breaker.record_failure(proxy_url, error_type="TIMEOUT")
                    return False
        except Exception as e:
            logger.error(f"âŒ VALIDATION ERROR: {e}")
    ...
    def _inject_proxy_session(self, proxy_url: str) -> str:
        ...
        try:
            ...
            logger.warning(f"âš ï¸ Session Injection Failed: {e}")
            return proxy_url

    def _launch_chrome_cdp(self) -> bool:
        """
        Lance Chrome avec CDP activÃ© sur le profil configurÃ© (CHROME_USER_DATA_DIR / CHROME_PROFILE_DIRECTORY).
        AppelÃ© automatiquement si aucun endpoint CDP ne rÃ©pond.
        """
        chrome_bin = settings.CHROME_BIN
        if not chrome_bin or not os.path.exists(chrome_bin):
            logger.error("Chrome binary not found â€” impossible de lancer Chrome automatiquement.")
            return False

        user_data_dir = getattr(settings, "CHROME_USER_DATA_DIR", None)
        profile_dir = getattr(settings, "CHROME_PROFILE_DIRECTORY", "Default")
        ...
        args = [
            ...
            "--disable-session-crashed-bubble",  # pas de dialog "Chrome ne s'est pas fermÃ© correctement"
            "--disable-restore-session-state",  # pas de restauration de session
            "--disable-features=ProfilePicker,ChromeWhatsNew",  # ProfilePicker bloque l'init CDP
            "about:blank",  # ouvre une page directement â†’ bypasse le sÃ©lecteur de profil
        ]
        ...
        if user_data_dir:
            ...
            logger.info(f"Chrome CDP : profil '{profile_dir}' dans '{user_data_dir}'")
            # Nettoyer le lock file stale (laisse par un kill brutal)
            lock_path = Path(user_data_dir) / profile_dir / "SingletonLock"
            if lock_path.exists():
                try:
                    lock_path.unlink()
                    logger.info("Lock file Chrome stale supprime.")
                except Exception:
                    pass
        else:
            logger.warning(
                "CHROME_USER_DATA_DIR non defini â€” Chrome lancera avec un profil temporaire."
            )

        try:
            if os.name == "nt":
                # Sur Windows, PowerShell Start-Process (SANS -NonInteractive) est la seule
                # mÃ©thode fiable pour lancer Chrome avec CDP depuis Python.
                # -NonInteractive prive Chrome de son window station â†’ Chrome se ferme.
                # cmd /c start et DETACHED_PROCESS ne lient pas le port CDP.
                def _ps_quote(s: str) -> str:
                    return '"' + s.replace('"', '`"') + '"'

                ps_arg_list = ", ".join(_ps_quote(a) for a in args[1:])
                ps_cmd = f"Start-Process {_ps_quote(chrome_bin)} -ArgumentList @({ps_arg_list})"
                subprocess.Popen(
            ...
            logger.info(f"Chrome CDP lance sur le port {self.cdp_port}.")
            return True
        except Exception as e:
            logger.error(f"Echec du lancement Chrome : {e}")
            return False

    async def connect(
        self, playwright, warmup_only: bool = False, specific_proxy: str = None
    ) -> bool:
        """
        ...
        async with self.context_lock:
            ...
            if not is_healthy:
                logger.error("ðŸ›‘ SELF-HEALING: Session Invalid/Expired (Maintenance Triggered).")
            ...
            if active_proxy_url:
                ...
                if not is_safe:
                    logger.critical(
                        f"ðŸ›‘ PATH REJECTED: Proxy failed validation for {self.user_id}."
                    )
                    return False

            # --- AUTO-LAUNCH CHROME SI AUCUN ENDPOINT CDP N'EST ACTIF ---
            endpoint_alive = False
            for ep in self.cdp_endpoints:
                try:
            ...
            if not endpoint_alive:
                logger.info("Aucun endpoint CDP actif â€” lancement automatique de Chrome...")
                launched = self._launch_chrome_cdp()
                if launched:
                    # Polling jusqu'a ce que le port CDP reponde (max 15s)
                    primary_ep = self.cdp_endpoints[0]
                    for _tick in range(15):
                        await asyncio.sleep(1)
                        try:
                            async with aiohttp.ClientSession() as _s:
                                async with _s.get(
                                    primary_ep + "/json/version",
                                    timeout=aiohttp.ClientTimeout(total=1),
                                ) as _r:
                                    if _r.status == 200:
                                        logger.info(f"Chrome CDP pret apres {_tick + 1}s.")
                                        break
                        except Exception:
                            pass
                else:
                    logger.error(
                        "Impossible de lancer Chrome. Verifiez CHROME_BIN et CHROME_USER_DATA_DIR."
                    )
            ...
            for endpoint in self.cdp_endpoints:
                ...
                for attempt in range(3):
                    try:
                        ...
                        self.latency_multiplier = (self.latency_multiplier * 0.7) + (
                            current_factor * 0.3
                        )
                        ...
                        if connection_time > 2.0 or self.latency_multiplier > 1.5:
                            logger.info(
                                f"ðŸ¢ NETWORK DEGRADED (Score: {self.network_quality_score:.2f}). Adjusting thresholds."
                            )
                            self.is_degraded_mode = True

                        # NOTE: Ne pas fermer les contextes existants sur une connexion CDP rÃ©elle.
                        # Sur un profil Chrome utilisateur, fermer tous les contextes ferme Chrome.
                        # On rÃ©utilise le contexte existant ou on en crÃ©e un nouveau.
                        ...
                        if active_proxy_url:
                            ...
                            if proxy_conf:
                                ...
                                logger.info(f"ðŸŒ Proxy active for {self.user_id}")

                        # Sur un profil Chrome rÃ©el (CDP), il faut rÃ©utiliser le contexte
                        # existant (qui contient les cookies de session).
                        # new_context() crÃ©e un contexte isolÃ© sans cookies â†’ prÃ©fÃ©rer l'existant.
                        if self.browser.contexts:
                            self.context = self.browser.contexts[0]
                            logger.info(
                                "CDP real profile: reusing existing browser context (session preserved)."
                            )
                        else:
                        ...
                        if warmup_only:
                            await self._execution_environment_preparation(
                                session_data, last_exit_status=last_exit_status
                            )
                            return True

                        logger.info(
                            f"âœ… Connected [ID: {self.user_id}] - Health: {self.health_score:.2f} - QoS Score: {self.network_quality_score:.2f}"
                        )
                        ...
                        self.circuit_breaker.record_failure(
                            endpoint, error_type="CONNECTION_REFUSED"
                        )
        ...
        try:
            ...
            for p in unavailable_markers:
                if await self.page.is_visible(p, timeout=500):
                    logger.warning(f"â˜£ï¸ SERVICE UNAAVAILABLE: {p}")
                    return False
            return True
        except Exception:
            return True
    ...
    async def handle_access_challenge(self) -> bool:
        ...
        try:
            ...
            if not settings.CAPTCHA_API_KEY:
                logger.warning("âš ï¸ Access Challenge detected but API Key missing.")
                logger.info("âœ‹ MANUAL INTERVENTION: 30s to resolve challenge...")
                await asyncio.sleep(30)
                return True

            logger.info("ðŸ§© Resolving Access Challenge...")
            await asyncio.sleep(5)

            if await self.check_service_availability():
                logger.info("âœ… Access Challenge Resolved.")
                return True

            return False
        except Exception as e:
            logger.error(f"âŒ Challenge Resolution Error: {e}")
            return False
        finally:
    ...
    async def smart_click(self, target_name: str, strategies: List[str]) -> bool:
        ...
        async def attempt_strategies():
            ...
            for selector in strategies:
                try:
                    if await self.page.is_visible(selector, timeout=2000):
                        ...
                        logger.info(f"âœ… Click success (Technical): {selector}")
                        return True
                except Exception:
                    domain = urlparse(self.page.url).netloc
                    self.circuit_breaker.record_failure(
                        "site_structure", is_site_error=True, target_domain=domain
                    )
                    continue

            # 2. Semantic Fallback (XPath - Resilient Navigation)
            logger.info(
                f"ðŸ§  Smart Click: Technical selectors failed. Attempting Semantic Fallback for '{target_name}'..."
            )
            ...
            if not relevant_keywords:
                relevant_keywords = semantic_keywords
                ...
                for template in xpath_templates:
                    try:
                        ...
                        if element and await element.is_visible():
                            logger.info(
                                f"ðŸ§  HEURISTIC CLICK: Found element via semantic match '{kw}'"
                            )
        ...
        # --- STRATÃ‰GIE DE DÃ‰GAGEMENT (OVERLAY KILLER) ---
        logger.warning(
            f"âš ï¸ Interaction blocked for '{target_name}'. Attempting de-cluttering..."
        )
        try:
                ...
                try:
                    ...
                    for el in elements:
                        if await el.is_visible():
                            logger.info(f"ðŸ§¹ De-cluttering: Removing obstruction ({obs})")
            ...
            logger.warning(f"âš ï¸ De-cluttering failed: {e}")

        # 3. Last Resort: Keyboard or Force JS
        if any(k in target_name.lower() for k in ["login", "search", "enter"]):
            logger.warning(
                f"âš ï¸ Smart Click failed for {target_name}. Attempting fallback (ENTER)."
            )
            try:
        ...
        logger.error(f"âŒ Interaction failed for {target_name}")
        return False

    async def _execution_environment_preparation(
        self, session_data: Dict, last_exit_status: str = "clean"
    ):
        """
        ...
        try:
            ...
            if not scenario_tag:
                ...
                logger.info(
                    f"ðŸŽ­ SCENARIO ASSIGNED (Contextual): {scenario_tag.upper()} [Geo: {geo}]"
                )
            ...
            if last_exit_status in ["crash", "detected", "timeout", "unknown"]:
                logger.info(f"ðŸš‘ Recovery Mode: Gentle preparation for {self.user_id}")
            ...
            logger.info(f"â˜• Environment Prep ({scenario_tag.upper()}): {len(steps)} steps.")

            for url, dwell_time in steps:
                try:
                    domain = urlparse(url).netloc
                    if not self.circuit_breaker.allow_request("site_check", target_domain=domain):
                        logger.warning(f"â© Skipping {domain} (Unstable).")
                        continue

                    logger.info(f"âž¡ï¸ Visiting: {url}")
                    ...
                    logger.warning(f"âš ï¸ Prep step failed ({url}): {step_err}")

        except Exception as e:
            logger.warning(f"âš ï¸ Global Prep Error: {e}")

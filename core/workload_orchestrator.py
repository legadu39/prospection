...
logging.basicConfig(
    level=logging.INFO, format="%(asctime)s | %(levelname)-8s | [DISPATCHER] %(message)s"
)
...
# Loaded from config/sponsors.json["yield_tiers"] — edit that file to change payouts.
_PARTNER_YIELD_TIERS_FALLBACK = {
...
def _load_yield_tiers() -> dict:
    sponsors_path = Path(__file__).resolve().parents[1] / "config" / "sponsors.json"
    try:
        with open(sponsors_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        tiers = data.get("yield_tiers")
        if tiers and isinstance(tiers, dict):
            return tiers
    except Exception as e:
        logger.warning(
            f"[YieldTiers] Impossible de charger sponsors.json: {e} — fallback hardcodé utilisé."
        )
    return _PARTNER_YIELD_TIERS_FALLBACK


PARTNER_YIELD_TIERS = _load_yield_tiers()
...
class ComputeGridOrchestrator:
    ...
    def _update_health_scores(self):
        ...
        try:
            ...
            now = time.time()
            self._velocity_exclusion = {
                k: v for k, v in self._velocity_exclusion.items() if v > now
            }
            ...
            for s_id in active_ids:
                ...
                if current_rate > 0.5:
                    logger.warning(
                        f"📉 PARTNER ALERT: Partner {s_id} links are failing (Error Rate: {current_rate:.1%}). Pausing traffic."
                    )

                # Preventive Pause if errors surge suddenly (+15% in one cycle)
                elif velocity > 0.15 and current_rate > 0.2:
                    logger.warning(
                        f"📉 VELOCITY ALERT: Partner {s_id} unstable (+{velocity:.1%} errors). Preventive pause for 10min."
                    )
                ...
                if nodes:
                    ...
                    for n in nodes:
                        try:
                            ...
                            n["monthly_limit_hard"] = self.db.to_decimal(
                                n.get("monthly_limit_hard", "20")
                            )
    ...
    def infer_process_type(self, data_packet: Dict[str, Any]) -> Optional[str]:
        ...
        if data_packet.get("intent") and "PROTOCOL_" in data_packet["intent"]:
            ...
            if any(x in raw_intent for x in ["PROP", "FUNDING", "CAPITAL"]):
                return "prop_firm"
            if any(x in raw_intent for x in ["SAAS", "TOOL", "CHART", "TRADINGVIEW"]):
                return "tradingview"
            if any(x in raw_intent for x in ["SECURITY", "LEDGER", "HARDWARE"]):
                return "ledger"
            if any(x in raw_intent for x in ["PASSIVE", "YIELD", "MERIA"]):
                return "meria"
            if any(x in raw_intent for x in ["CRYPTO", "CHAIN", "BINANCE"]):
                return "binance"
        ...
        text_corpus = (
            str(data_packet.get("comment_text") or "")
            + " "
            + str(data_packet.get("ai_draft") or "")
        ).lower()
        scores = Counter()
    ...
    def attempt_atomic_allocation(
        self, packet_id: str, protocol: str, candidates: List[Dict], lead_score: float = 0.5
    ) -> Optional[Dict]:
        """
        ...
        eligible_candidates.sort(
            key=lambda x: (self._calculate_ucb1_score(x), -float(x.get("verified_count_month", 0))),
            reverse=True,
        )
    ...
    def _adjust_batch_size(self, success_rate: float):
        ...
        logger.info(
            f"🎛️ PID: Success Rate {success_rate:.2f} -> Batch Size adjusted to {self.current_batch_size}"
        )
    ...
    def process_dispatch_cycle(self):
        ...
        logger.info(
            f"🚚 Processing Agency Batch {batch_id}: {len(batch)} leads (Limit: {self.current_batch_size})..."
        )
        stats = Counter()
        ...
        for data_packet in batch:
            try:
                ...
                winner = self.attempt_atomic_allocation(
                    data_packet["id"], workload_type, candidates, quality_score
                )

                if winner:
                    payout = winner.get("estimated_payout", 0)
                    logger.info(
                        f"✅ DISPATCH {data_packet['id'][:8]} -> {winner['label']} (Est. Gain: {payout}€, Q-Score: {quality_score:.2f})"
                    )
                    stats["dispatched"] += 1
                    ...
                    try:
                        with self.db.session() as conn:
                            conn.execute(
                                "UPDATE leads SET status='WAITING_PARTNER', updated_at=? WHERE id=?",
        ...
        if total_processed > 0:
            success_rate = (
                stats["dispatched"] + stats["skipped"] + stats["waiting"]
            ) / total_processed

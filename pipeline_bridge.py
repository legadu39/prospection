...
KEYWORD_WEIGHTS = {
    ...
    "urgent": 10, "bloqué": 10, "problème": 8, "aide": 7,
    "argent": 5, "crypto": 8, "investir": 6, "tuto": 4,
    "trading": 15, "prop": 12, "challenge": 12, "funded": 10,
    "capital": 8, "funding": 8, "bitcoin": 6, "analyse": 5,
    "lien": 5, "mp": 3, "dispo": 2,

    # SIGNALS NEGATIFS (Noise / Spam / Hors-Cible IOBSP)
    "fake": -20, "arnaque": -50, "police": -100,
    "scam": -50, "bot": -10, 
    
    # HARD BLOCK BANCAIRE (Conformité V3)
    "banque": -50, "crédit": -100, "prêt": -100, 
    "taux": -20, "emprunt": -100, "immobilier": -20,
    "bourso": -50, "fortuneo": -50  # Nettoyage Legacy
}
    ...
    try:
        ...
        for lead in raw_leads:
            ...
            if isinstance(created_at, str): # Fallback si parsing bizarre
                try: created_at = float(created_at)
        ...
        top_ids = [l['id'] for l in top_leads]
...
async def _process_single_lead_task(lead: dict, hybrid_gen: HybridGenerator):
    ...
    try:
        ...
        if len(context_txt) < 3:
            with db.session() as conn:
                conn.execute("UPDATE leads SET status='SKIP', updated_at=? WHERE id=?", (time.time(), lead_id))
            return
        ...
        if INSTANT_CLOSE_REGEX.search(context_txt):
            logger.info(f"⚡ FAST-TRACK: Lead {lead_id[:8]} identifié comme 'Haute Intention'. Bypass IA.")
        ...
        raw_resp, ai_source = await hybrid_gen.generate(final_prompt, lead.get('comment_text', ''), headless_connector=None)
        ...
        if status not in ["QUALIFIED", "SKIP"]: status = "SKIP"
        ...
        if status == "QUALIFIED" and keyword_learner and hasattr(keyword_learner, 'update_word_hit'):
        ...
        is_transient = isinstance(e, (asyncio.TimeoutError, ConnectionError)) or "timeout" in str(e).lower()
        
        current_info = lead.get('ai_process_info')
        info_dict = {}
        try:
             info_dict = json.loads(current_info) if isinstance(current_info, str) else (current_info or {})
        except: pass
...
async def run_ai_processing_cycle():
    ...
    logger.info(f"🧠 Pipeline: Processing Batch {len(batch)} leads (Target Size: {int(current_size)})...")
    ...
    if abs(correction) > 2.0:
        logger.debug(f"🎛️ PID Regulator: Batch {int(current_size)} -> {int(new_size)} (Error: {error:.2f}s)")
...
async def run_pipeline():
    ...
    if hasattr(rag, 'initialize'):
        if asyncio.iscoroutinefunction(rag.initialize): await rag.initialize()
        else: rag.initialize()
            
    recover_stale_leads()
    
    asyncio.create_task(watchdog_zombie_task())

...
from core.time_manager import CircadianCycle  # noqa: E402
...
class TikTokTopologyMapper:
    ...
    def __init__(self):
        ...
        self.signal_performance: Dict[str, Dict[str, float]] = defaultdict(
    ...
    async def run(self):
        logger.info("📡 Topology Mapper V37.3 Started (Strategy: Arbitrage Global).")
            ...
            while self.running:
                ...
                try:
                    ...
                    if not selected_signal:
                        logger.warning("⚠️ All signals quarantined. Forced Standby 10min.")
                        await asyncio.sleep(600)
                    ...
                    search_url = (
                        f"https://www.tiktok.com/search?q={q_encoded}&t=video&publish_time=1"
                    )

                    logger.info(
                        f"🔎 Scanning Topology: '{self.current_signal}' (Weight: {self.signal_performance[self.current_signal][str(datetime.utcnow().hour)]:.2f})"
                    )
                    ...
                    for _ in range(scrolls):
                        await humanizer.scroll_organic(
                            intensity="high" if random.random() > 0.7 else "medium"
                        )
    ...
    async def _handle_network_response(self, response: Response):
        url = response.url
        if not any(
            ep in url for ep in ["/api/search/item", "/api/item_list", "/api/recommend/item_list"]
        ):
            return
    ...
    def _calculate_node_quality(self, item: dict, reputation_data: dict) -> int:
        ...
        if following > 0:
            ...
            if ratio > 5000:
                score += 15  # Super Editor / Celebrity
            elif 0.5 < ratio < 5.0:
                score += 10
            elif ratio < 0.1:
                score -= 20
            elif ratio > 100:
                score -= 5  # Potential Bot
    ...
    async def _process_batch(self, data: dict):
        ...
        for item in items:
            ...
            reputation = await asyncio.to_thread(
                self.db.get_author_reputation, hashed_node_id, "tiktok"
            )
            if reputation["status"] == "BANNED" and reputation["expiry_ts"] > time.time():
                continue
            ...
            if node_quality < 20:
                await asyncio.to_thread(
                    self.db.update_author_reputation, hashed_node_id, "tiktok", -5
                )
                continue
            ...
            if is_vip:
                ...
                try:
                    logger.info(
                        f"🕸️ TRIANGULATION: Ajout du VIP {hashed_node_id} à la queue d'exploration virale."
                    )
                    await asyncio.to_thread(
            ...
            candidates.append(
                (item_id, item, text, final_capacity, node_quality, hashed_node_id, is_vip)
            )
        ...
        for cid, raw_item, text, capacity, quality, hashed_author_id, is_vip in candidates:
            ...
            if inserted:
                ...
                await asyncio.to_thread(
                    self.db.update_author_reputation,
                    hashed_author_id,
                    "tiktok",
                    trust_bonus,
                    "TRUSTED",
        ...
        if new_nodes_count > 0:
            logger.info(
                f"⚡ {new_nodes_count} Nodes discovered (VIPs: {super_editors_found}). Signal '{self.current_signal}'."
            )
            ...
            if self.current_signal:
                ...
                self.signal_performance[self.current_signal][current_hour] = min(
                    10.0, current_val * boost
                )
    ...
    def _handle_failure(self):
        ...
        if self.current_signal:
            self.consecutive_failures[self.current_signal] = (
                self.consecutive_failures.get(self.current_signal, 0) + 1
            )
            if self.consecutive_failures[self.current_signal] >= 3:
                logger.warning(
                    f"📉 Signal '{self.current_signal}' degraded (3 failures). Quarantined 4h."
                )
                self.quarantine_list[self.current_signal] = time.time() + (4 * 3600)

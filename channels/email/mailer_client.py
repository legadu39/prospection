...
class NurturingBot:
    ...
    def _check_pressure(self, user_email: str, hours: int = 24) -> bool:
        ...
        try:
            ...
            with self.db.session() as conn:
                ...
                """, (author_hash, cutoff)).fetchone()
                
                count = res['cnt'] if hasattr(res, 'keys') else res[0]
                
                if count > 0:
                    logger.info(f"⏳ Pression Marketing: {user_email} déjà contacté il y a moins de {hours}h. Skip.")
                    return False
                return True
    ...
    def _predict_optimal_send_time(self, email: str) -> str:
        ...
        free_providers = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'orange.fr', 'wanadoo.fr']
    ...
    def _get_dynamic_template(self, product_key: str, next_offer: str, user_context: str) -> tuple:
        ...
        tone_style = "color: #4CAF50;" if user_context == "B2C_PERSONAL" else "color: #2E86C1;" # Vert vs Bleu Pro
    ...
    def _dispatch_email(self, to_email, subject, html_content, tags=None):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 200 <= response.status_code < 300:
                    ...
                    if self.db:
                        try:
                            ...
                            author_hash = self.db._hash_identity(to_email)
                ...
                logger.warning(f"⚠️ Échec envoi (Tentative {attempt+1}/{max_retries}). Pause {wait_time}s... Erreur: {e}")
    ...
    def send_cross_sell_sequence(self, user_email, first_product_bought):
        ...
        subject, raw_html = self._get_dynamic_template(first_product_bought, next_offer, user_profile)
        final_html = raw_html.replace("{{LINK}}", affiliate_link)

        # 5. Envoi Résilient
        self._dispatch_email(user_email, subject, final_html, tags=["cross-sell", first_product_bought])
